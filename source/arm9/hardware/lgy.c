...
void LGY_prepareLegacyMode(bool gbaBios)
{
	...
	{
		FIL f;
		if(f_open(&f, "sdmc:/Emerald Enhanced.gba", FA_OPEN_EXISTING | FA_READ) == FR_OK)
		{
		...
		g_saveSize = 1024 * 128;
		if(f_open(&f, "sdmc:/Emerald Enhanced.sav", FA_OPEN_EXISTING | FA_READ) == FR_OK)
		{
			UINT read;
